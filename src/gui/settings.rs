...
impl SettingsObserver {
    pub fn new(path: Option<PathBuf>) -> Self {
        ...
        let mut o = Self { path: path.clone(), last_modified: None, settings: AppSettings::default() };
        o.load(); o
    }
    pub fn load(&mut self) {
        if let Ok(meta) = std::fs::metadata(&self.path) {
            if let Ok(modified) = meta.modified() { self.last_modified = Some(modified); }
        }
    ...
    pub fn check_reload(&mut self) -> bool {
        std::fs::metadata(&self.path).and_then(|m| m.modified()).map_or(false, |t| Some(t) != self.last_modified).map_or(false, |changed| { if changed { self.load(); } changed })
    }
    ...
    pub fn settings_mut(&mut self) -> &mut AppSettings { &mut self.settings }
}
